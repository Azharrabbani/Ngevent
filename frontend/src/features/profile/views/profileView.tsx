...
export default function ProfileView() {
    ...
    const attendee = useGetCurrentAttendeeProfile()
    ...
    if (user.role === "user") {
        if (attendee.loading) return <div>Loading...</div>

        return (
            <ProfileContainer role={user.role}>
                <h1 className="font-bold text-2xl mb-6">Profile</h1>
                <AttendeeProfileForm profile={attendee.profile}/>
            </ProfileContainer>
        )
    }

    if (user.role === "event organizer") {
        if (organizer.loading) return <div>Loading...</div>

        return (
            <ProfileContainer role={user.role}>
                <h1 className={`font-bold text-2xl mb-6 flex items-center gap-1`}>
                    Profile
                    {organizer.profile?.status.status === "approved" && 
                ...
                <OrganizerProfileForm profile={organizer.profile}/>
            </ProfileContainer>
        )
    }
